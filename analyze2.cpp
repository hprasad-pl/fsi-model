...
void analyze2()
{
  std::vector<std::string> filenames {"events_p=0.01.root", "events_p=0.008.root", "events_p=0.012.root"};
  ...
  for(int file_index=0; file_index<filenames.size(); file_index++) {
    ...
    for(Int_t event_index=0; event_index<nentries; event_index++){
      ...
      h[file_index]->Fill(count_interactions);
    }
  ...
  SetHistAttributes(h[1], xlabel, ylabel, LineStyles::Solid, Colors::kMagenta1, 3 );
  SetHistAttributes(h[2], xlabel, ylabel, LineStyles::Solid, Colors::kOrange1, 3 );
  ...
  // Create a legend using the function
    TLegend* legend = CreateLegendAutoPosition(h[0], "p = 0.01");

    // Add entries to the legend
    legend->AddEntry(h[0], "Default", "l"); // "l" for line
    legend->AddEntry(h[1], "p * 0.8", "l");
    legend->AddEntry(h[2], "p * 1.2 ", "l");
    // Draw the legend
    legend->Draw();
}
